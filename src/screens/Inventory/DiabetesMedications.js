    ...
    >
      ...
      <View style={styles.statusContainer}>
        ...
        <TouchableOpacity 
          ...
          onPress={() => navigation.navigate('MedicationInfo', { id: medication.id })}
        >
...
const DiabetesMedications = () => {
  ...
  const fetchMedications = async () => {
    try {
      setLoading(true);
      const response = await axios.get(buildApiUrl('/api/medications'));
  ...
  const confirmDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(buildApiUrl(`/api/medications/${medicationToDelete}`));
      ...
      >
        {filteredMedications.map((medication) => (
          <MedicationCard
            ...
            onEdit={() => navigation.navigate('EditMedicationScreen', { 

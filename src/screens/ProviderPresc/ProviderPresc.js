...
const ProviderPresc = () => {
  ...
  const [filteredPatients, setFilteredPatients] = useState({ new: [], existing: [] });
  ...
  useEffect(() => {
    if (searchQuery) {
      const filtered = {
        new: groupedPatients.new.filter(patient =>
          patient.full_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
          patient.id.toString().includes(searchQuery)
        ),
        existing: groupedPatients.existing.filter(patient =>
          patient.full_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
          patient.id.toString().includes(searchQuery)
        )
      };
  ...
  const renderPatientCard = (patient, type) => (
    <TouchableOpacity
      ...
      onPress={() => navigation.navigate('ViewPatientPresc', { 
        patientId: patient.id,
        providerId: providerId  // Add this line to pass providerId
      })}
    >
      <LinearGradient
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <View style={styles.welcomeContainer}>
          <Text style={styles.welcomeText}>Prescriptions</Text>
          <Text style={styles.subtitleText}>
            Manage your patients' prescriptions
          </Text>
        </View>
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        {searchQuery ? (
          <View style={styles.section}>
            ...
            {filteredPatients.new.length > 0 || filteredPatients.existing.length > 0 ? (
              <>
                {filteredPatients.new.map(patient => renderPatientCard(patient, 'new'))}
                {filteredPatients.existing.map(patient => renderPatientCard(patient, 'existing'))}
              </>
            ) : (

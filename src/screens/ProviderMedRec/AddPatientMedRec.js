...
const AddPatientMedRec = () => {
  ...
  const [record, setRecord] = useState({
    ...
    ldl: '',
    triglyceride: '',
    next_checkup_date: moment().add(1, 'month').format('YYYY-MM-DD'),
  });
  ...
  const handleMetricChange = (field, value) => {
    setRecord(prev => {
      ...
      if (value === '') {
        updated[field] = '';
        // Clear BMI if either height or weight becomes empty
      ...
      // Auto-calculate BMI when either height or weight changes
      if (field === 'height' || field === 'weight') {
        const height = parseFloat(field === 'height' ? value : updated.height);
        const weight = parseFloat(field === 'weight' ? value : updated.weight);
        
        // Only calculate if both values are valid numbers and height is not zero
        if (!isNaN(height) && !isNaN(weight) && height > 0) {
          // Convert height to meters if needed (assuming input is in meters)
          const heightInMeters = height;
          const bmi = weight / (heightInMeters * heightInMeters);
          
          // Only set BMI if the calculation is valid
...
const createMedicalRecord = async () => {
  try {
    setLoading(true);
    const response = await axios.post(buildApiUrl('/api/medical-records'), record);
    setSuccessMessage('Medical record created successfully');

    // Trigger immediate update of parent view
    if (route.params?.onRecordAdded) {
      route.params.onRecordAdded();
    }

    // Add a small delay before navigation to ensure updates are visible
    setTimeout(() => {
      navigation.goBack();
    }, 1000);
  } catch (error) {
    console.error('Error creating record:', error.response || error);
    setError(error.response?.data?.message || 'Failed to create medical record');
  } finally {
    setLoading(false);
  }
};

  const onNextCheckupDateChange = (event, selectedDate) => {
    setShowNextCheckupPicker(false);
    if (selectedDate) {
      setRecord(prev => ({
        ...prev,
        next_checkup_date: moment(selectedDate).format('YYYY-MM-DD')
      }));
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {showNextCheckupPicker && (
        <DateTimePicker
          value={new Date(record.next_checkup_date)}
          mode="date"
          onChange={onNextCheckupDateChange}
...
export default AddPatientMedRec;